...
class ListService {
    ...
    async getAllTodos(userId: number, pagination?: PaginationParams, filters?: ListFilters): Promise<PaginatedResult<ToDoList> | string> {
    ...
    async updateTodo(userId: number, id: number, todoData: UpdateListDto): Promise<ToDoList | string> {
    ...
    async markTodoAsCompleted(id: number, userId: number): Promise<ToDoList | string> {
      const existingTodo = await this.listRepository.findById(id, userId);
